...
public class Getopt extends Object
	...
	/**
	  ...
	  */
	private ResourceBundle _messages = PropertyResourceBundle.getBundle("gnu/getopt/MessagesBundle",
			Locale.getDefault());
	...
	public Getopt(String progname, String[] argv, String optstring, LongOpt[] long_options,
		...
		if (System.getProperty("gnu.posixly_correct",null) == null)
			...
			_messages = PropertyResourceBundle.getBundle("gnu/getopt/MessagesBundle",Locale.US);
			}
				...
				if (pfound.has_arg != LongOpt.NO_ARGUMENT)
					...
					if (opterr)
						...
						if (argv[optind - 1].startsWith("--"))
							...
							System.err.println(MessageFormat.format(_messages.getString("getopt.arguments1"),
									msgArgs));
							}
						// +option or -option
						else
							...
							Object[] msgArgs = { progname,new Character(argv[optind - 1].charAt(0)).toString(),
									pfound.name };
							System.err.println(MessageFormat.format(_messages.getString("getopt.arguments2"),
									msgArgs));
							}
						}
	...
	public int getopt()
		...
		if ((long_options != null)
				&& (argv[optind].startsWith("--") || (long_only && ((argv[optind].length() > 2) || (optstring.indexOf(argv[optind].charAt(1)) == -1)))))
			{
			...
			if (!long_only || argv[optind].startsWith("--")
				...
				if (opterr)
					...
					if (argv[optind].startsWith("--"))
						...
						System.err.println(MessageFormat.format(_messages.getString("getopt.unrecognized"),
								msgArgs));
						}
					else
						{
						Object[] msgArgs = { progname,new Character(argv[optind].charAt(0)).toString(),nextchar };
						System.err.println(MessageFormat.format(_messages.getString("getopt.unrecognized2"),
								msgArgs));
						}
					}
					...
					if ((posixly_correct) && optarg.equals("--"))
						...
						if (optind == argv.length)
							...
							if (opterr)
								...
								// 1003.2 specifies the format of this message
								Object[] msgArgs = { progname,new Character((char) c).toString() };
								System.err.println(MessageFormat.format(_messages.getString("getopt.requires2"),
										msgArgs));
								}
