...
public final class EnigmaWriter {
	...
	protected void populateInformation() {
		...
		oi.backgroundColor = ARGBtoRGBA(((Color) ig
				.get(PGameInformation.BACKGROUND_COLOR)).getRGB());
	...
	protected void populateSettings() {
		...
		og.forceSoftwareVertexProcessing = ig
				.get(PGameSettings.FORCE_SOFTWARE_VERTEX_PROCESSING);
		...
		og.colorOutsideRoom = ARGBtoRGBA(((Color) ig
				.get(PGameSettings.COLOR_OUTSIDE_ROOM)).getRGB());
		og.setResolution = ig.get(PGameSettings.SET_RESOLUTION);
		og.colorDepth = ProjectFile.GS_DEPTH_CODE.get(
				ig.get(PGameSettings.COLOR_DEPTH)).byteValue();
		og.resolution = ProjectFile.GS_RESOL_CODE.get(
				ig.get(PGameSettings.RESOLUTION)).byteValue();
		og.frequency = ProjectFile.GS_FREQ_CODE.get(
				ig.get(PGameSettings.FREQUENCY)).byteValue();
		...
		og.letF4SwitchFullscreen = ig
				.get(PGameSettings.LET_F4_SWITCH_FULLSCREEN);
		...
		og.gamePriority = ProjectFile.GS_PRIORITY_CODE.get(
				ig.get(PGameSettings.GAME_PRIORITY)).byteValue();
		og.freezeOnLoseFocus = ig.get(PGameSettings.FREEZE_ON_LOSE_FOCUS);
		og.loadBarMode = ProjectFile.GS_PROGBAR_CODE.get(
				ig.get(PGameSettings.LOAD_BAR_MODE)).byteValue();
		// populateImage((BufferedImage)
		// ig.get(PGameSettings.FRONT_LOAD_BAR),og.frontLoadBar,false);
		// populateImage((BufferedImage)
		// ig.get(PGameSettings.BACK_LOAD_BAR),og.backLoadBar,false);
		og.showCustomLoadImage = ig.get(PGameSettings.SHOW_CUSTOM_LOAD_IMAGE);
		// populateImage((BufferedImage)
		// ig.get(PGameSettings.LOADING_IMAGE),og.loadingImage,false);
		og.imagePartiallyTransparent = ig
				.get(PGameSettings.IMAGE_PARTIALLY_TRANSPARENTY);
		...
		og.includeFolder = ProjectFile.GS_INCFOLDER_CODE.get(ig
				.get(PGameSettings.INCLUDE_FOLDER));
		...
		// All this shit is just to write the icon to a temp file and provide
		// the filename...
	...
	protected void populateSprites() {
		...
		for (int s = 0; s < size; s++) {
			...
			os.shape = ProjectFile.SPRITE_MASK_CODE.get(is.get(PSprite.SHAPE)); // 0*=Precise,
																				// 1=Rectangle,
																				// 2=Disk,
																				// 3=Diamond
			...
			os.bbMode = ProjectFile.SPRITE_BB_CODE.get(is.get(PSprite.BB_MODE)); // 0*=Automatic,
																					// 1=Full
																					// image,
																					// 2=Manual
			...
			if (os.maskShapeCount == 0)
				continue;

			os.maskShapes = new Polygon.ByReference();
			Polygon[] opl = (Polygon[]) os.maskShapes
			...
			for (int j = 0; j < opl.length; j++) {
				...
				if (opl[j].pointCount == 0)
					continue;

				System.out.println("Test1");

				opl[j].points = new Point.ByReference();
				Point[] oppl = (Point[]) opl[j].points
						.toArray(opl[j].pointCount);
		...
		for (int f = 1; f < size; f++) {
			...
			if (ifont.characterRanges.size() > 0) {
				...
				for (int i = 0; i < ofgrl.length; i++) {
					...
					ofgrl[i].rangeMax = cr.properties
							.get(PCharacterRange.RANGE_MAX);
					ofgrl[i].glyphs = populateGlyphs(
							ifont.getAWTFont(screenRes), ofgrl[i].rangeMin,
							ofgrl[i].rangeMax,
							(Integer) ifont.get(PFont.ANTIALIAS));
				}
				...
				ofgrl[0].rangeMin = 32;
				ofgrl[0].rangeMax = 127;
				ofgrl[0].glyphs = populateGlyphs(ifont.getAWTFont(screenRes),
						32, 127, (Integer) ifont.get(PFont.ANTIALIAS));
	...
	private static void populateGlyph(Glyph og, java.awt.Font fnt, int c, int aa) {
		...
		og.baseline = r.getY(); // bump image down X pixels (usually negative,
								// since baseline is at bottom)
		og.advance = gv.getGlyphPosition(gv.getNumGlyphs()).getX(); // advance X
																	// pixels
																	// from
																	// origin
		...
		bi.getRGB(0, 0, bi.getWidth(), bi.getHeight(), rasterRGB, 0,
				bi.getWidth());
	...
	protected void populateObjects() {
		...
		for (int s = 0; s < size; s++) {
			...
			if (oo.mainEventCount == 0)
				continue;

			oo.mainEvents = new MainEvent.ByReference();
			MainEvent[] ooil = (MainEvent[]) oo.mainEvents
					.toArray(oo.mainEventCount);
	...
	protected void populateRooms() {
		...
		for (int s = 0; s < size; s++) {
			...
			or.backgroundColor = ARGBtoRGBA(((Color) is
					.get(PRoom.BACKGROUND_COLOR)).getRGB());
			...
			or.deleteUnderlyingObjects = is
					.get(PRoom.DELETE_UNDERLYING_OBJECTS);
			...
			if (or.backgroundDefCount != 0) {
				...
				for (int bd = 0; bd < obdl.length; bd++) {
					...
					obd.foreground = ibd.properties
							.get(PBackgroundDef.FOREGROUND);
					obd.x = ibd.properties.get(PBackgroundDef.X);
					obd.y = ibd.properties.get(PBackgroundDef.Y);
					obd.tileHoriz = ibd.properties
							.get(PBackgroundDef.TILE_HORIZ);
					...
					obd.backgroundId = toId(
							ibd.properties.get(PBackgroundDef.BACKGROUND), -1);
				}
			}
			...
			for (org.lateralgm.resources.sub.Instance ii : is.instances)
				if (toId(ii.properties.get(PInstance.OBJECT), -1) != -1)
					ri.add(ii);
			...
			if (or.instanceCount != 0) {
				...
				for (org.lateralgm.resources.sub.Instance ii : ri) {
					...
					if (scale.getX() != 1.0)
						sb.append("image_xscale="
								+ String.valueOf(scale.getX()) + ";\r\n");

					if (scale.getY() != 1.0)
						sb.append("image_yscale="
					...
					if (!Color.WHITE.equals(color))
						sb.append("image_blend=make_color_rgb("
								+ String.valueOf(color.getRed()) + ","
								+ String.valueOf(color.getGreen()) + ","
								+ String.valueOf(color.getBlue()) + ");"
								+ "\r\n");

					int alpha = ii.getAlpha();
			...
			if (or.tileCount != 0) {
				...
				for (int t = 0; t < otl.length; t++) {
					...
					ot.backgroundId = toId(it.properties.get(PTile.BACKGROUND),
							-1);
	...
	public static ArrayList<LibAction> getQuestionLibActions() {
		...
		for (Library lib : LibManager.libs)
			for (LibAction act : lib.libActions)
				if (act.question && act.execType == Action.EXEC_CODE)
					ala.add(act);
		return ala;
	}
}
